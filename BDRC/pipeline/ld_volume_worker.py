...
from .parquet_writer import S3ParquetWriter
...
class LDVolumeWorker:
    ...
    def __init__(self, cfg: PipelineConfig, s3: S3Context, volume_id: str, tasks):
        ...
        self.prefetch = Prefetcher(cfg, self.s3ctx, tasks, self.q_prefetcher_to_decoder)
        self.decode = Decoder(cfg, self.q_prefetcher_to_decoder, self.q_decoder_to_gpu_pass_1)
        self.batch = LDGpuBatcher(cfg, self.q_decoder_to_gpu_pass_1, self.q_post_processor_to_gpu_pass_2, self.q_gpu_pass_1_to_post_processor, self.q_gpu_pass_2_to_post_processor)
