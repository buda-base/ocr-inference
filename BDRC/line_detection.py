...
def generate_guid(clock_seq: int) -> UUID:
    """
...
def optimize_countour(cnt: npt.NDArray, e: float = 0.001) -> npt.NDArray:
    """
    ...
    Returns:
        Optimized contour
    """
    epsilon = e * cv2.arcLength(cnt, closed=True)
    return cv2.approxPolyDP(cnt, epsilon, closed=True)
...
def get_rotation_angle_from_lines(
    ...
    high_angles = [x for x in angles if abs(x) != RIGHT_ANGLE and x > (RIGHT_ANGLE - max_angle)]

    if len(low_angles) > len(high_angles) and len(low_angles) > 0:
        mean_angle = float(np.mean(low_angles))
    # check for clockwise rotation
    elif len(high_angles) > 0:
        mean_angle = -(RIGHT_ANGLE - float(np.mean(high_angles)))
    else:
        mean_angle = 0.0
...
def calculate_rotation_angle_from_lines(
    ...
    *,
    ...
    if debug_angles:
        print(f"All Angles: {angles}")  # noqa: T201
...
def build_line_data(contour: npt.NDArray, *, optimize: bool = True) -> Line:
    """
...
def build_raw_line_data(image: npt.NDArray, line_mask: npt.NDArray) -> tuple[npt.NDArray, npt.NDArray, list, float]:
    """
    ...
    """
    if len(line_mask.shape) == RGB_NDIM:
    ...
    line_contours = [x for x in line_contours if cv2.contourArea(x) > MIN_CONTOUR_AREA]
...
def filter_line_contours(image: npt.NDArray, line_contours: Sequence, threshold: float = 0.01) -> list:
    """
    ...
    for _, line_cnt in enumerate(line_contours):
        ...
        if w > image.shape[1] * threshold and h > MIN_LINE_HEIGHT:
...
def extract_line_images(
    image: npt.NDArray, line_data: list[Line], default_k: float = 1.7, bbox_tolerance: float = 3
) -> list[npt.NDArray]:
    """
...
def get_line_threshold(line_prediction: npt.NDArray, slice_width: int = 20) -> float:
    """
    Calculate threshold for line sorting based on detected lines.

    This function generates n slices (of n = steps) width the width of slice_width across
    the bbox of the detected lines. The slice with the max. number of contained contours
    is taken to be the canditate to calculate the bbox center of each contour and take
    the median distance between each bbox center as estimated line cut-off threshold to
    sort each line segment across the horizontal.

    Note: This approach might turn out to be problematic in case of sparsely spread
    line segments across a page.
    ...
    """
    if len(line_prediction.shape) == RGB_NDIM:
    ...
    if len(sorted_list) > 0:
        ...
        if n_contours == 0:
            ...
            line_threshold = float(np.median(y_points) // n_contours) if y_points else 0.0
    else:
        line_threshold = 0.0
...
def sort_bbox_centers(bbox_centers: list[tuple[int, int]], line_threshold: float = 20) -> list[list[tuple[int, int]]]:
    """
    ...
    for i in range(len(bbox_centers)):
        if len(tmp_line) > 0:
            ...
            # y_diff = abs(tmp_line[0][1] - bbox_centers[i][1])
            """
...
def group_line_chunks(
    sorted_bbox_centers: list[list[tuple[int, int]]], lines: list[Line], *, adaptive_grouping: bool = True
) -> list[Line]:
    """
...
def sort_lines_by_threshold(
    ...
    *,
    ...
    line_treshold = get_line_threshold(line_mask) if calculate_threshold else threshold
